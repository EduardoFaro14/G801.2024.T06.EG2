...
class hotelManager:
    ...
    def validar_formato_fecha(self, cadena):
        if len(cadena) == 10 and cadena[2] == '/' and cadena[5] == '/' and cadena[0].isdigit() and cadena[
            1].isdigit() and cadena[3].isdigit() and cadena[4].isdigit() and cadena[6:].isdigit():
            return True
        else:
            return False
    ...
    def guardar_reserva_en_archivo(self, localizador, idCard, creditCardNumber, arrival, nameSurname, phoneNumber,
                                   roomType, numDays):
        # Nombre del archivo donde se guardarán las reservas
        nombreArchivo = (r"C:\Users\eduardo faro jr\OneDrive\Documentos\3 curso 2 cuatri"
    ...
    def room_reservation(self, creditCardNumber, idCard, nameSurname, phoneNumber, roomType, arrival, numDays):
        ...
        self.guardar_reserva_en_archivo(localizador, idCard, creditCardNumber, arrival, nameSurname
                                        , phoneNumber, roomType, numDays)
        return localizador
    ...
    def guest_arrival(self, input_file):
        ...
        try:
            with open(r"C:\Users\eduardo faro jr\OneDrive\Documentos\3 curso 2 cuatri"
                      r"\EG2\src\main\python\json_files\reservas.json", 'r') as f:
        ...
        room_type = reserva2["RoomType"]

        if localizer != localizer2:
            raise hotelManagementException("El localizador de reservaf2 no se corresponde con "
        ...
        hotel = hotelStay(idCard2, localizer2, numDays, room_type)
        arrival = hotel.arrival
        departure = hotel.departure
        room_key = hotel.room_key
        dd, mm, yyyy = map(int, arrivalReserva.split('/'))
        if dd != arrival.day or mm != arrival.month or yyyy != arrival.year:
            raise hotelManagementException("La fecha de llegada de reservaf2 no "
                                           "coincide con la fecha de llegada en reservaf1")
        self.guardar_reserva2_en_archivo(localizer2, idCard2, room_type, arrival, departure, room_key)
        return room_key
